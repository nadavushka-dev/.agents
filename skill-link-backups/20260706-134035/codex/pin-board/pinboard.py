...
board rules: file-per-item, never auto-delete decision/tech-debt/shortcut,
always stamp updatedBy, atomic writes.
"""
import argparse
import glob
import json
...
from _common import (
    DEFAULT_DIR, PRUNABLE, STATUS, TYPE, REL, REQUIRED,
    load, parse_iso, iso_now, write_item, item_path,
)
...
def cmd_lint(a):
    ...
    for it in items.values():
        ...
        for l in it.get("links", []) or []:
            ...
            elif tgt not in items:
                errors.append(f"{i}: dangling link → '{tgt}' (no such item)")
        par = it.get("parent")
        if par and par not in items:
...
def cmd_gc(a):
    ...
    if to_delete:
        print(f"\nFully-done task subtrees to delete ({len(to_delete)}):")
        for i in to_delete:
            print(f"  {i}")
    else:
        print("\nNo fully-done epic/task/subtree to delete.")
    if manual:
        print(f"\nResolved decisions/tech-debt/shortcuts (manual-drop only, NOT deleted):")
        for i in manual:
            print(f"  {i}")

    if a.execute and to_delete:
        for i in to_delete:
            os.remove(item_path(a.dir, i))
        print(f"\n✓ deleted {len(to_delete)} item(s).")
    elif to_delete:
...
def main():
    ...
    p = sub.add_parser("gc", help="delete fully-done task subtrees (dry-run unless --execute)")
    p.add_argument("--dir", default=DEFAULT_DIR)
